...
export default function Courses() {
  ...
  const { mapProvider, map, crs } = useMap(getMap, shallow);
  ...
  return (
    <div className="pb-4">
      ...
      <FilePicker
        ...
        onSelect={loadCourse}
      />
      {map && mapProvider && crs && selectedCourse && (
        <CourseLayer
          eventName={eventName}
          mapScale={mapScale}
...
function getMap({ mapProvider, map }) {
  return { mapProvider, map, crs: mapProvider.getCrs() };
}
